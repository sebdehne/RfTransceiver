...
angular.module('project.controllers', [])
  ...
  .controller('Ventilation', function ($scope, $location, VentilationService) {
    $scope.go = function (url) {
      $location.path(url);
    };
    ...
    $scope.current_level = "unknown";
    VentilationService.getStatus(function (status) {
      $scope.current_level = round((status['value']) * 100 / 255);
    });

    $scope.send = function (value) {
      VentilationService.sendLevel(value);
    };
  })
